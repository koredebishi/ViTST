...
def construct_demogr_description(static_demogr, length):
    ...
    if static_demogr[2] or static_demogr[3]:
        ...
        if static_demogr[3]:
            icu = "surgical ICU"
    desc.append(f"went to the {icu} {round(static_demogr[3])} hours after hospital admit, had stayed there for {static_demogr[4]} hours, and has the physiological data within {length} hours")
...
def construct_image(
    ...
    # Iterate over patients to construct ImageDict_list
    for idx, p in tqdm(enumerate(Pdict_list)):

        pid = p['id'].split("\\")[-1].split(".psv")[0].strip() # Extract patient ID
        length = p['length']
        static_demogr = p['extended_static']
        demogr_desc = construct_demogr_description(static_demogr, length)
    ...
    # Save the constructed ImageDict_list as a .npy file
    print(len(ImageDict_list))
    np.save(f'./processed_data/ImageDict_list.npy', ImageDict_list)
    print(f"Saved data in ImageDict_list.npy")
